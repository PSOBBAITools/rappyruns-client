...
- 応答到着時に ptr がまだ同じなら `*ghost*` に設定 (古い取得結果を捨てる)。C# 版 (S36) は ptr ではなくロード番号 (新しいロード・ロビー・ゲーム終了のたびに増える) で比べる。ptr だけでは、同じアドレスに読み込まれた次のクエストや再起動後のゲームへの遅い応答を区別できないため。ピンセット取得も同じ (`Refetch` も番号を進める)。また同じ ptr でもクエスト名が違えば新しいロードとみなす (ロビーのフレームを見逃した場合)。
