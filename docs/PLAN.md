...
- **P8 完了 (2026-09-24): `v1.0.0` を `PSOBBAITools/rappyruns-client` に公開。** 待ち期間は置かなかった: 当時の継続利用者は開発者を除き 5 人で、主力の 1 人は既に v0.61.0、1 人は自動更新していない 0.44.0。保護なしで切り替わりうるのは 2 人だけで、個別に案内できる規模だったため。C# のソースは公開リポジトリの `csharp` ブランチ (main は Lisp の履歴のまま)、更新履歴の版数は `desktop/VERSION` を見る
- 切り替え後の追跡: サーバーの `run_diagnostics.client_version` で各ユーザーの版数を見る
