...
| `settings.setTriggerLog` | `{enabled}` | Notice? | Trigger log. On: `start-trigger-log` then `trigger-log-on-rotated (trigger-log-on <path>) <old path> <MiB>` (the Lisp `:trigger-log-on <path>` plus the rotated file, S41); off: close. |
